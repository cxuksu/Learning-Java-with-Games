...
public class RuleBase {
    ...
    public void flipPiece() {
        for (int idx = 0; idx < rules.size(); idx++) {
            ...
            //aRule.setFlipFlag(true); // "best" requires measureing num flips
            if (aRule.isValid(newRow, newCol)) {
                aRule.flipPiece();
            }
    ...
    public boolean isRuleValid() {
        ...
        for (int idx = 0; idx < rules.size(); idx++) {
            if ((rules.get(idx)).isValid(newRow, newCol)) {
                theRule = rules.get(idx);
                totalNumFlip = totalNumFlip + theRule.getNumFlip();
                ruleValid = true;
            }
        }
        return ruleValid;
