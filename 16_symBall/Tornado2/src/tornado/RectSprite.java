/*
 * RectSprite.java - A class that implements a rectangle sprite.
 */
package tornado;
...
public class RectSprite extends AbsSprite2D {
    ...
    public void initSprite(int section) {
        ...
        setX(midx + getMajorR());
        setY(midy);
        setWidth((int) (2 * Math.random() * Consts.RECTSIZE));
        setHeight((int) (2 * Math.random() * Consts.RECTSIZE));
        currAngle = (int) (Math.random() * 360);
        moveAngle = (int) (Math.random() * 10);
        //setColor(ranGrayColor());
        setActive(true);
        setVisible(true);
    }
