/*
 * DToSSprite.java - A class that converts two integers in decimal to
 * binary signed notation and renders calculations dynamically.
 * it looks like a sprite in a game so that name it as DToSSprite.
 */
package sumtwos;
 ...
 * @author cxu
 */
public class DToSSprite extends AbsSprite {

    int inInt, inBase, remainder, loop = 1;
    final int ARYLEN = 8;
    final int lineH = 20, gapH = 2, gapW = 5, intW = 60;
    final int topMargin = 300, leftMargin = 100, middleGap = 150;
    int x, y, xLoop2;
    ...
    private PropertyChangeSupport dToSDone;
    ...
    public void initSprite() {
        dToSDone = new PropertyChangeSupport(this);
    ...
    private void toSigned(Graphics2D g2d) {
        int loopOne = 1; // to keep the first loop for re-paint loop one
        if (loop == 1) {
            ...
            if (twoDone) {
                ...
                dToSDone.firePropertyChange("dToSDone", false, true);
            }
        }
    }
    ...
    public void renderConversion(Graphics2D g2d, int loop, int count,
        ...
        if (loop == 2) {
            ...
            // because the quoLen is in changing, (x2 - ...) is easy to find out
            // the drawing starting point than (x + ...)
        }
    ...
    public void addPropertyChangeListener(PropertyChangeListener pcl) {
        dToSDone.addPropertyChangeListener(pcl);
    }
}
