/*
 * UpRightRule.java - A rule that places a piece on the down-left position
 * to flip the opponent pieces on the up-right direction.
 */
package rule;
 ...
 * @author cxu
 */
public class UpRightRule extends AbsRule {
    ...
    public boolean verify(int newRow, int newCol, Color theColor) {
        boolean flag = false;
        if (((newRow - 1) >= 0) && ((newCol + 1) < Consts.MAXCELL)
            ...
            if (!(pieceBoard[newRow - 1][newCol + 1].getColor().equals(theColor))) {
                ...
                int row = newRow - 2;
                int col = newCol + 2;
                while ((row >= 0) && (col < Consts.MAXCELL)
